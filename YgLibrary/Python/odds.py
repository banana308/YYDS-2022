...
def get_odds():
    ...
    if num_outcomeId==0:
        ...
        for i in range(0,len(results['data']['marketList'])):
            ...
            cc_list.append(results['data']['marketList'][i]['outcomeList'][0][0]['id'])
...
def get_submitbet(i):
    ...
    if code == 0:
        # print(str(name)+"投注成功")
        print(str(i) + "投注成功:"+dd_list[i] + "(" + ee_list[i] +")"+"-"+ cc_list[i]+" 投注orderNo：:"+str(orderNo))
        print(
            "---------------------------------------------------------分割线------------------------------------------------------------------")
    else:
        ...
        exit()
    #print(results)
    print("{已下注成功用户："+name,"已成功投注项："+str((i+1))," 正在投注项："+"未开启多线程"," 未投注项："+str((num_outcomeId-i-1))+" 投注orderNo："+str(orderNo)+"}"+"\n")
    ...
    if i==(int(num_outcomeId)-1):
        ...
        f = open(path01, 'a')
        f.write("投注球类："+yyds+"-"+str(matchId)+"-"+ "\n"+"投注盘口数量："+str(num_outcomeId))
        #写入比赛ID
